...
class FilterComponent(
    ...
    init {
        ...
        scope.launch {
            ...
            val factionOptions =
                listOf(SelectOption.All) +
                    factions.await().map {
                        SelectOption.Item(it.id, it.name, it.imageUri)
                    }
            val attributesOptions =
                listOf(SelectOption.All) +
                    attributes.await().map {
                        SelectOption.Item(it.id, it.name, it.imageUri)
                    }
            val specialityOptions =
                listOf(SelectOption.All) +
                    specialities.await().map {
                        SelectOption.Item(it.id, it.name, it.imageUri)
                    }
            val rarityOptions =
                listOf(SelectOption.All) +
